...
class SearchingControllerTest < ActionController::TestCase
  ...
  test 'search issues without conditions' do
    ...
    assert_equal "Issue", assigns[:results].klass.name, 'Wrong class name!'
    assert_equal 2, assigns[:results].entries.count, 'Wrong total count! Should equal 2!'
  ...
  test 'search issues with project condition' do
    ...
    assert_equal 1, assigns(:results).entries.count, 'Wrong total count! Should equal 1!'
    entries = assigns(:results).entries.collect(&:id)
  ...
  test 'search as admin' do
    ...
    allow_user_admin
    get :esearch, esearch: 'test', klass: "Issue"
    assert_equal 5, assigns(:results).entries.count, 'Wrong total count! Should equal 5!'
  end
  ...
  test 'search as user with permission' do
    ...
    @request.session[:allowed_to_private] = true
    get :esearch, esearch: 'test', klass: "Issue"
    assert_equal 3, assigns(:results).entries.count, 'Wrong total count! Should equal 2!'
  end
  ...
  test 'search as allowed user should be possible to find private issues' do
    @request.session[:allowed_to_private] = true
    get :esearch, esearch: 'test', klass: "Issue", is_private: 'true'
    assert_equal 1, assigns(:results).entries.count, 'Wrong total count! Should equal 2!'
  end
  ...
  test 'search admin should see all issue(private, public)' do
    ...
    assert_equal 10, assigns(:results).entries.count, 'Should found 10 issue! - all issues'
  end
  ...
  test 'should return all projects when query *' do
    ...
    assert_equal Project.count, assigns(:results).entries.count, 'Wrong total count! Should return projects!'
    assert_equal 4, assigns(:results).entries.count, 'Wrong total count! Should return 4!'
  end
  ...
  test 'search issue with attachments content' do
    ...
    assert_equal 10, assigns(:results).entries.count, 'Wrong total count! Should return 10(issues test and attach test!'
  end

  test 'search wikipages' do
    allow_user_admin
    get :esearch, esearch: 'test', klass: "WikiPage"
    assert_equal 1, assigns(:results).entries.count, 'Wrong count of WikiPages! Should be 1!'
  end
end
